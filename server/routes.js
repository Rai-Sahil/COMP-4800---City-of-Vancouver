  ...
  mainConnection.query(query, function (err, result) {
    ...
    for (let i = 0; i < result[0].length; i++) {
      ...
      if (fs.existsSync(`public/artistImages/${result[0][i].uuid}/`)) {
        let imagePaths = fs.readdirSync(
          `public/artistImages/${result[0][i].uuid}/`,
          { withFileTypes: true }
        );
      ...
      let partialArtist = {
        uuid: result[0][i].uuid,
        name: result[0][i].name,
        cultural: result[0][i].cultural,
        preference: result[0][i].preference,
        genre: result[0][i].genre,
        image: image,
      };
...
app.get("/artists/single", (req, res) => {
  ...
  mainConnection.query(query, function (err, result) {
    ...
    mainConnection.query(
      approvedDateQuery,
      (approvedDateErr, approvedDateResult) => {
        if (approvedDateErr) {
        ...
        artist.approvedDate =
          approvedDateResult.length > 0
            ? approvedDateResult[0].approvedDate
            : null;

        // Handling images
        artist.images = [];
        let imagePaths = fs.readdirSync(`public/artistImages/${artistId}/`, {
          withFileTypes: true,
        });
...
function notifyAdminForApplication(headerBarDOM, data) {
  ...
  if (data[0] && data[0].length > 0) {
    ...
    let hamburger =
      headerBarDOM.window.document.getElementsByClassName("hamburger")[0];
